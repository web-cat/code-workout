...
class UploadRosterController < ApplicationController
  ...
  def handle_submit
    if @uploaded_roster.save
      respond_to do |format|
        format.js do
          flash[:notice] = "Created #{@uploaded_roster.users_created.count} new users and enrolled #{@uploaded_roster.users_enrolled.count} users."
        end
      end
    else
      respond_to do |format|
        format.js do
          flash[:error] = 'You must select exactly one column for e-mail and either full name or first and last name.'
