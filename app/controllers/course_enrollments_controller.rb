class CourseEnrollmentsController < ApplicationController
  before_action :set_course_enrollment,
  ...
  def index
    if cannot? :index, CourseEnrollment
      ...
      redirect_to root_path,
        notice: 'Unauthorized to view course enrollments' and return
    end
    print "CAN ACCESS"
  ...
  def show
    if cannot? :show, @course_enrollment
      redirect_to root_path,
        notice: 'Unauthorized to view course enrollment' and return
    end
  ...
  def new
    if cannot? :new, CourseEnrollment || !user_signed_in?
      redirect_to root_path,
        notice: 'Unauthorized to create course enrollments' and return
    else
  ...
  def edit
    if cannot? :edit, CourseEnrollment || !user_signed_in?
      redirect_to root_path,
        notice: 'Unauthorized to edit course enrollments' and return
    end
  end
  ...
  def update
    if cannot? :update, CourseEnrollment || !user_signed_in?
      redirect_to root_path,
        notice: 'Unauthorized to update course enrollments' and return
    end
    if @course_enrollment.update(course_enrollment_params)
      redirect_to @course_enrollment,
        notice: 'Course enrollment was successfully updated.'
    else
      render action: 'edit'
    end
  end
  ...
  def destroy
    ...
    redirect_to course_enrollments_url,
      notice: 'Course enrollment was successfully destroyed.'
  end
  ...
  private
    ...
    def course_enrollment_params
      params.require(:course_enrollment).permit(
        :user_id, :course_offering_id, :course_role_id)
    end
end
